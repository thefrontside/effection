...
export function main(body: (args: string[]) => Operation<void>): Promise<void> {
  return api.lookup(global).main(body);
}

// this is the core core main function
global.decorate(api, {
  async main([body]) {
...
//     return yield* callcc<Exit>(function* (resolve) {
//       // action will return shutdown immediately upon resolve, so stash
//       // this function in the exit context so it can be called anywhere.
//       yield* ExitContext.set(resolve);
