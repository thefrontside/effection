...
Add `Context` to the end of the context reference. This makes it easy to identify context references. For example, a config context might be called `ConfigContext`.
...
Provide an `init` function for initializing context. You'll typically call this once at the beginning of your program.
...
Provide a `use` helper for reading the context. Provide a helper for retrieving the value from the context with `use` prefix.
