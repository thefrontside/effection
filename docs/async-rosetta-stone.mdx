...
promise. An abort signal is the only way to ask the promise to stop — and asking
is not the same as it having stopped. If your cleanup depends on the work being
finished, keep the promise and wait for it during teardown with
[`ensure()`][ensure]. Register that `ensure()` before creating the signal, since
destructors run in reverse order.

```javascript
import { ensure, until, useAbortSignal } from 'effection';
...
function* upload(body) {
  ...
  yield* ensure(function*() {
    try {
      ...
      // abort rejects with AbortError; teardown should not throw
    }
  });

  let signal = yield* useAbortSignal();

  request = fetch('/uploads', { method: 'POST', body, signal });

  return yield* until(request);
}
```
