...
just three primitive operations_: `action()` and `resource()`.
...
bundled right alongside the logit to exit it. As a result, and we'll
...
`action()` and `resource()`. Composability is the key to everything.
