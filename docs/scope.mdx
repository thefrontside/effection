...
You can use `try/finally` for synchronous cleanup. If you need `yield*` during
cleanup, use [`ensure()`][ensure], otherwise the halt will be lost and
execution will continue past the operation you were halting.

``` javascript
import { ensure, run, sleep, suspend } from 'effection';
