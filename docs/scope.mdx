...
accepts an abort signal to cancel itself when needed. This is the standard
way to call a Promise-based API so that its work belongs to the current
operation:
...
- `fetch()` returns an ordinary promise, and promises are eager: the request
  is already in flight.
- [`until()`][until] lets Effection wait on that promise as an operation.
- [`useAbortSignal()`][use-abort-signal] connects the promise-based work to
  the current Effection lifetime. If the operation fails or is halted while
  the request is active, the signal is aborted.
- The promise API must honor the signal you hand it. Effection does not
  intrinsically cancel arbitrary promises; it can only tell an API that
  accepts an `AbortSignal` to stop.
- The response body is consumed inside the operation, so what escapes is an
  ordinary value rather than an effect that depends on a signal which is
  about to be aborted.

>💡Using `fetch()` often? [`@effectionx/fetch`](/x/fetch), one of the
> community-created Effection Extensions, packages this pattern into an
> Effection-native `fetch` operation with cancellation tied to the current
> scope. To adapt any other cancellable Promise API, use the pattern above.
