...
In JavaScript, developers rarely have to think about memory allocation within a function because the lifetime of memory
allocation is bound to the scope of the function. When a function is finished, it's scope is torn down by the JavaScript 
runtime and all of the memory allocated to variables in function's scope can be released safely. Binding memory
management to scope gives developers the freedom to focus on their application instead worrying about leaking memory. 

Structured concurrency establishes the same relationship between scope and operations. Every Effection operation is 
...
> 💡 You might assume that Effection makes everything asyncronous which is incorrect. Effection is built on 
